...
#for(i in 1:length(cont_dgpis)){
  ...
  for (j in 1:100){
    ...
    results <- weighting_regression(y, z, X, results, true_ate)
    print(paste("DGP: ", i, "Iteration:", j))
  }
  
  all_results[[paste0("dgp", dgp_index)]] <- results
  saveRDS(results, paste0(PATH_RESULTS, "2026-04-15_weighting_",dgp_index,".RData"))
}

saveRDS(all_results, paste0(PATH_RESULTS, "2026-04-15_weighting_all.RData"))