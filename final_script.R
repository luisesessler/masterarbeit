...
for (i in 1:n_iter){
  # Simulierte Daten
  data <- generate_data(linear = TRUE)
  X <- data[, c("x1", "x2", "x3", "x4", "x5", "x6")]
  
  # Overall ATE
  ate_complete <- mean(data$ite)
  
  # How many treated & non-treated?
  perc_treated <- nrow(data[data$z == 1, ])/nrow(data)
  perc_control <- nrow(data[data$z == 0, ])/nrow(data)
  
  
  #TODO: Split training/test
  percentage_train <- 0.7
  train <- sample(nrow(data), percentage_train * nrow(data), replace = FALSE)
  
  data_train <- data[train,]
  data_test <- data[-train,]
  
  # ATEs
  ate_train <- mean(data$y_1[train] - data$y_0[train])
  ate_test <- mean(data$y_1[-train] - data$y_0[-train])
  
  #--------------------------------------------------------------
  #--------------------------------------------------------------
  # Propensity Scores (using BART)
  ps_formula_rightside <- paste0("x", 1:6, collapse = " + ")
  ps_formula <- as.formula(paste0("z ~ ", ps_formula_rightside))
  ...
  #---- Fit model
  bart_fit_one_model <- bartc(response = data$y, treatment = data$z, confounders = X, subset = train, method.rsp = "bart", method.trt = "none",
  ...
  formula_rightside <- paste0("x", 1:6, collapse = " + ")
  ...
  ci_length_two_models_train <- mean(ci_length_two_models_all[train])
  ci_length_two_models_test <- mean(ci_length_two_models_all[-train])
  
  # Coverage
  is_covered_two_models_train <- ifelse(credible_intervals_two_models_all[,1] <= ate_train &
                                        ate_train <= credible_intervals_two_models_all[,2], 1, 0)
  is_covered_two_models_test <- ifelse(credible_intervals_two_models_all[,1] <= ate_test &
                                          ate_test <= credible_intervals_two_models_all[,2], 1, 0)
  ...
  results_train$coverage_results_train$BART_two_models[[length(results_train$coverage_results_train$BART_two_models) + 1]]  <- is_covered_two_models_train
  ...
  results_test$coverage_results_test$BART_two_models[[length(results_test$coverage_results_test$BART_two_models) + 1]]  <- is_covered_two_models_test
  ...
  fit_ps_glm <- bartc(response = data$y, treatment = data$z, confounders = X, subset = train, method.rsp = "bart", method.trt = "glm",
                      estimand = "ate", keepTrees = TRUE)
  ...
  formula_rightside_ps <- paste0("x", 1:6, collapse = " + ")
  ps_formula <- as.formula(paste0("z ~ ", formula_rightside_ps))
  ...
  match_regression_formula <- as.formula(paste0("y ~ z + ", formula_rightside_ps))
...
toc()
avg_test <- get_averages(results_test)
...
generate_data <-  function(linear) {
  ...
  logit_p <- -0.5 + 0.01 * x1 - 0.03 * x2 + 0.1 * x4 - 0.2 * x5 + 0.02 * x6
  
  # Transform to Probability
  p_score <- 1 / (1 + exp(-logit_p))
  
  z <- rbinom(n1, size = 1, p_score)
  
  # check p_score distribution
  #hist(p_score)
  ...
  # Create dataframe
  df <- data.frame(x1, x2, x3, x4, x5, x6, z, p_score, y_1, y_0, y, ite)
  return(df)
}