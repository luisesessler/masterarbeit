...
for(i in 1:3){
  ...
  for (j in 1:100){
    ...
    results <- bart_ps_glm_new(y, z, X, results, true_ate)
    print(paste("DGP: ", i, "Iteration:", j))
  }
  
  all_results[[paste0("dgp", dgp_index)]] <- results
  saveRDS(results, paste0(PATH_RESULTS, "2026-03-19_ps-glm_",dgp_index,".RData"))
}

saveRDS(all_results, paste0(PATH_RESULTS, "2026-03-19_ps-glm_dgp1-3.RData"))