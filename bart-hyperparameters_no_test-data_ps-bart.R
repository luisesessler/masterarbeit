...
for(i in 12){
  ...
  for (j in 2:20){
    for (l in 1:nrow(hyper_bart)){
      ...
      metrics_run <- bart_s_learner(y, z, X, ps_as_covariate = TRUE, method_trt = "bart",
  ...
  write.csv(df_tuning_results, paste0(PATH_RESULTS, "2026-04-28_tuning_NO-CV_dgp60_ps-bart_sigma", j,".csv"))
}
