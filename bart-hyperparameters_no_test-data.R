...
df_tuning_results_k_ntrees <- data.frame(
  dgp = character(),
  metric = character(),
  model = character(),
  iteration = numeric(),
...
for(i in 13){
  ...
  for (j in 1:20){
    ...
    metrics_run <- bart_s_learner(y, z, X, k = hyperparams$k, n_trees = hyperparams$n_trees, 
                                  nu = hyperparams$nu, true_ate = true_ate)
    ...
    print(paste("dgp:", i, "iteration:", j, "k:", hyperparams$k, "n.trees:", hyperparams$n_trees, "nu:", hyperparams$nu))
  }
  }
}


bart_s_learner <- function(y, z, X, true_ate, method_trt = "none", ps_as_covariate = FALSE, k, n_trees, nu, testdata = X){
...
bart_s_learner_glm <- function(y, z, X, true_ate, testdata = X){
  
  bart_fit <- bartc(response = y, treatment = z, confounders = X, 
                    method.rsp = "bart", method.trt = "glm", commonSup.rule = "sd", 
...
bart_t_learner <- function(y, z, X, true_ate, testdata = X){
  ...
  bart_fit_treated <- bart2(formula = bart_formula, data = treated, keepTrees = TRUE, combineChains = TRUE, n.chains = n_chains, n.samples = n_samples)
  bart_fit_control <- bart2(formula = bart_formula, data = control, keepTrees = TRUE, combineChains = TRUE, n.chains = n_chains, n.samples = n_samples)
