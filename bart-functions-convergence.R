...
bart_two_models <- function(y, z, X, results, true_ate, testdata = X){
  ...
  bart_fit_treated <- bart2(formula = bart_formula, data = treated, keepTrees = TRUE, combineChains = TRUE, n.chains = 4)
  bart_fit_control <- bart2(formula = bart_formula, data = control, keepTrees = TRUE, combineChains = TRUE, n.chains = 4)
  ...
  return(bart_fit_treated, bart_fit_control)
}
...
bart_ps <- function(y, z, X, results, true_ate, trt_method, testdata = X){
  ...
  bart_fit <- bartc(response = y, treatment = z, confounders = X,
                    ...
                    n.chains = 4) #TODO: warum steht da estimand = "ATE"
