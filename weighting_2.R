...
#for(i in 1:length(cont_dgpis)){
  ...
  for (j in 1:100){
    ...
    metrics_run <- weighting_regression_glm(y, z, X, results, true_ate)
  ...
  write.csv(df_tuning_results, paste0(PATH_RESULTS, "2026-04-23_weighting_basic",".csv"), row.names = FALSE)
}

weighting_regression_glm <-  function(y, z, X, results, true_ate, newdata = X){
  ...
  ps <- predict(fit_ps_scores, type = "response")
